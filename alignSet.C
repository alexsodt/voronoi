...
float getXYChi2Float( float *xyz1, 
			float *xyz2,
			int nat )
{	
...
//		D,E,F,G,H,I,C0,C1,C2 );
	...
	for( int i = 0; i < nat; i++ )
	{
		double ovec[3] = {xyz2[3*i+0], xyz2[3*i+1], xyz2[3*i+2] };
		...
		if( prod_quat.z * ovec[2] < 0 )
			prod_quat.z *= -1;
		float newdr[3] = { prod_quat.x - xyz1[3*i+0], prod_quat.y - xyz1[3*i+1], prod_quat.z - xyz1[3*i+2] };


		chi2 += newdr[0]*newdr[0]+newdr[1]*newdr[1]+newdr[2]*newdr[2];
	}
