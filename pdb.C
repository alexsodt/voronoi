...
*/
void printATOM( FILE *toFile, int bead, int res, struct atom_rec *atrec, double aux )
{
	fprintf(toFile,"ATOM  ");
	char tbuf[256];
	sprintf(tbuf, "%d", bead );
...
//	fprintf(toFile, " " );
	...
	for( int x = 0; x < 8 - strlen(tbuf); x++ )
		fprintf(toFile," ");
	fprintf(toFile,"%s",tbuf);
	fprintf(toFile,"       %.2lf  1.00\n", aux);
}

void printATOM( char *toFile_in, int bead, int res, struct atom_rec *atrec, double aux )
{
