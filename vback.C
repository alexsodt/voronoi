...
//			avz /= nind;
			...
			if( colorByCluster )
			{
				fread( theCluster, nleaflet, sizeof(int), cFile ); 
				printf("read: %d\n", nleaflet );
				...
				for( int i = 0; i < nind; i++ )
				{
					if( leaflet_link[i] >= 0 )
					{
						int ind = theCluster[leaflet_link[i]];

						if( ind >= 0 )
						{
							printf("got one type %d ind %d\n", inds[i].type, leaflet_link[i] );
						}
						if( ind >= 0 )
							indexToColor( ind, colors+3*leaflet_link[i] ); 
						else
						{
							if( inds[i].type == chlAtCode )
							{
								colors[3*leaflet_link[i]+0] = 0.0;
								colors[3*leaflet_link[i]+1] = 0.0;
								colors[3*leaflet_link[i]+2] = 0.0;
							} 
							else
							{
								colors[3*leaflet_link[i]+0] = 1.0;
								colors[3*leaflet_link[i]+1] = 1.0;
								colors[3*leaflet_link[i]+2] = 1.0;
							}
						}
					}
				}
